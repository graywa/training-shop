...
import { addGoods, updateCartGoods } from '../../store/cartSlice'
...
const ProductPage = ({ category }) => {
  ...
  let { name, price, discount, rating, sizes, reviews, images, material } =
    useSelector((state) => {
  ...
  const getPriceWidthDisc = (price) => {
    if (discount)
      return (price - (parseInt(discount.slice(1)) / 100) * price).toFixed(2)
    return price
  }
  const addGoodsToCart = (id, name, size, color, photo, price) => {
    setGoodsInCart(true)
    const oneGoods = {id, name, size, color, photo, price, quantity: 1}
    dispatch(addGoods({ oneGoods }))
  }
                ...
                <div className='detail__description'>
                  <div className='colors-block'>
                    ...
                    <div className='colors-block__list'>
                      {images.map((el, ind) => {
                        if (images[ind].color !== images[ind - 1]?.color)
                          return (
                            <img
                              key={el.id}
                              src={`${imgHost}${el.url}`}
                              ...
                              className={
                                color === el.color
                                  ? 'colors-block__list-item active'
                                  : 'colors-block__list-item'
                              }
                  ...
                  <div className='price-block'>
                    <div className='price-block__count'>
                      {discount ? (
                        <div>
                          <span className='price-block__count_old'>
                            $ {price}
                          </span>
                    ...
                    >
                      {goodsInCart ? 'REMOVE FROM CART' : 'ADD TO CART'}
                    </button>
